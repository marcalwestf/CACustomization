...
//	private int			i_multiplier = 10;
	...
	{		
		...
		try
		{
			...
			pstmt.setInt(3, m_C_BPartnerPayment_ID);
			if (!isMultiCurrency)
...
//		paymentTable.setColumnClass(i, BigDecimal.class, true, names.get(i++));      	//  8-Multiplier
	...
	public Vector<Vector<Object>> getInvoiceData(boolean isMultiCurrency, Object date, IMiniTable invoiceTable)
	{
		 ...
		 * 
		 SELECT i.DateInvoiced,i.DocumentNo || ',' || coalesce(poreference,'') as documentno,i.C_Invoice_ID,c.ISO_Code,
...
//		i_multiplier = isMultiCurrency ? 10 : 8;
	...
	public String saveData(int m_WindowNo, Object date, IMiniTable payment, IMiniTable invoice, String trxName)
	{
		...
		if ( m_C_Charge_ID > 0 && unmatchedApplied.compareTo(Env.ZERO) != 0 )
		{
			...
			aLine.setC_BPartner_ID(m_C_BPartnerPayment_ID);
