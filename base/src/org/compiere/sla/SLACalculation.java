 ...
 */
public class SLACalculation extends SLACriteria
{

	/**
	 * 	DeliveryAccuracy
	 */
	public SLACalculation ()
	{
		super ();
	}	//	DeliveryAccuracy
	...
	{
		String sql = goal.getPA_SLA_Criteria().getSQLStatement();
		int counter = 0;
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				BigDecimal MeasureActual = rs.getBigDecimal(3);
				Timestamp MovementDate = rs.getTimestamp(4);
				String Description = rs.getString(6) + ": " + rs.getTimestamp(5);
				if (goal.isDateValid(MovementDate))
				{
					MSLAMeasure measure = new MSLAMeasure(goal, MovementDate, 
						MeasureActual, Description);
					measure.setLink(table_ID, record_ID);
					if (measure.save())
						counter++;
				}
			}
	...
	public BigDecimal calculateMeasure (MSLAGoal goal)
	{
		...
		if (count != 0 && goal.getPA_SLA_Criteria().getAggregationType().equals("AVG"))
