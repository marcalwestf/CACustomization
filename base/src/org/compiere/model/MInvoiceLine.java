...
public class MInvoiceLine extends X_C_InvoiceLine implements DocumentReversalLineEnable
{
	...
	public static MInvoiceLine getOfOrderLine (MOrderLine oLine)
	{
		if (oLine == null)
			return null;
		final String whereClause = I_C_OrderLine.COLUMNNAME_C_OrderLine_ID+"=?";
		List<MInvoiceLine> list = new Query(oLine.getCtx(),I_C_InvoiceLine.Table_Name,whereClause,oLine.get_TrxName())
		...
		if (list.size() > 0) {
			retValue = list.get(0);
			if (list.size() > 1)
				s_log.warning("More than one C_InvoiceLine of " + oLine);
		}
