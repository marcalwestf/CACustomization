...
public class FinReport extends FinReportAbstract {
	...
	private void insertLineDetail() {
		...
		String levelno = isDeleteEmptyLines()?">=0":"<>0";
		StringBuilder deleteReportLines = new StringBuilder ("DELETE FROM T_Report WHERE ABS(LevelNo) " + levelno)
	...
	private void insertLineSource(MReportLine reportLine) {
		...
		for(MReportSource source : reportLine.getSources()) {
			...
			if (!(Util.isEmpty(getListTrx()) 
					&& getListTrx().equals(MReport.LISTTRX_Yes)) && !source.getC_ElementValue().isSummary() && reportLine.getSources().length==1
					&& source.getC_ElementValue_ID() > 0) {
				
			}
			if (source.getC_ElementValue_ID() > 0 && !source.getC_ElementValue().isSummary() && reportLine.getSources().length==1 ) {	
				...
				if(!Util.isEmpty(getListTrx()) 
					...
					insertLineTrx(reportLine, variable, 0, whereReportLine + getWhereClauseForTrx());
