...
public class ValuationEffectiveDate extends ValuationEffectiveDateAbstract {
	...
	private void updateCostAndQtyonhand() throws Exception{
		//  Update Prices
		ArrayList<Object> params = new ArrayList<>();
		params.add(getDateValue());
		StringBuffer update = new StringBuffer("UPDATE T_InventoryValue iv "
				+ "SET qtyOnhand = "
				+ "(select sum(movementqty) from m_Transaction t "
				+ " INNER JOIN M_Locator l on t.m_Locator_ID=l.M_Locator_ID where l.M_Warehouse_ID=iv.m_warehouse_ID AND M_Product_ID=iv.m_Product_ID"
				+ " AND t.movementdate <=?) ");
			update.append(" WHERE iv.AD_PInstance_ID=").append(getAD_PInstance_ID());
			commitEx();
		//	Update
		DB.executeUpdateEx (update.toString(), params.toArray(), get_TrxName());
		StringBuffer updateCost = new StringBuffer("UPDATE T_InventoryValue iv "
				+ " set CumulatedAmt = qtyonhand*costamt ");
		update.append(" WHERE iv.AD_PInstance_ID=").append(getAD_PInstance_ID());
		DB.executeUpdateEx (updateCost.toString(), get_TrxName());
	}
