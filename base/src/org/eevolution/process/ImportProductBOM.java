...
public class ImportProductBOM extends SvrProcess {
	...
	private MPPProductBOM getMPPProductBOM(X_I_Product_BOM importBOM) {
		...
		if(importBOM.getPP_Product_BOM()!=null) {
			return new MPPProductBOM(Env.getCtx(), importBOM.getPP_Product_BOM_ID(),get_TrxName());
		}
	...
	private MPPProductBOMLine importBOMLine(MPPProductBOM bom,
		...
		bomLine.setLine((bom.getLines().length+1)*10);
		bomLine.saveEx();
		return bomLine;
	}
